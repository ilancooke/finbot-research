...
- For daily-grain research panels that need filing-effective fundamentals, use the shared `finbot_features.asof.asof_merge_fundamentals` helper when available. It attaches the latest fundamentals row with `effective_date <= date` and avoids future leakage; do not hand-roll a looser merge.
...
- Keep `fundamental-research-v0` outputs under `data/research/fundamental_research_v0`; this command may run exploratory filing-effective fundamental feature diagnostics, bucket diagnostics, candidate rules, scorecard v0, feasibility/holding-period summaries, and v1-readiness recommendations, but must not create production signals, dashboard views, model-training inputs, provider ingestion, portfolio construction, or live-trading logic.
- For `fundamental-research-v0` only, use numbered parquet files plus Markdown reports. Do not write CSV convenience copies or metadata JSON sidecars for that workflow unless the convention is explicitly revisited.
- Keep `fundamental-research-v0` memory bounded by using a filing-effective event panel: one row per `symbol,effective_date`, joined to forward labels on the same date. Do not expand the full daily label history with all fundamental feature columns unless the user explicitly asks for that heavier diagnostic.
- Keep `fundamental-research-v0` baselines as eligible-universe baselines for the same horizon and analysis scope; never compare a bucket, rule, or scorecard label to itself. V1 recommendation should stay conservative until label names, downside behavior, and stability are coherent.
- Keep `fundamental-research-v0` Markdown reports traceable: each report should include an `Output File Guide` section listing its backing parquet outputs. Keep `12c_fundamental_scorecard_relabeling_recommendation.parquet` as the explicit v0.1 relabeling handoff before any v1 freeze.
- Keep fundamentals v0.1 relabeling dimensional rather than composite: use separate quality score, opportunity score, risk label, and data-quality flag fields. Do not collapse defensive quality, speculative upside, rebound risk, and data quality into one score.
